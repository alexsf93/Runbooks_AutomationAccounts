<#
.SYNOPSIS
Application Records Report with certificates and their expiration dates, with colors according to expiration proximity.

.DESCRIPTION
This script retrieves application records and their certificates from Microsoft Graph API,
and generates an HTML report highlighting certificates nearing expiration, color-coded by remaining days.

.CONFIGURATION PARAMETERS
- GraphClientId: ID of the application registered in Azure AD.
- GraphTenantId: Azure AD tenant ID.
- GraphSecret: Application secret.
- Correo_No-Reply: SMTP credentials.
- recipient: Email recipients (one or more).
- useAttachment: 1 to send HTML as attachment and in the body, 0 for body only.
