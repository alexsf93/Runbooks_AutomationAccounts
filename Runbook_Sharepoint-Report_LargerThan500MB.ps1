<#
.SYNOPSIS
Generates a report of SharePoint files larger than 500MB using Microsoft Graph API.

.DESCRIPTION
This script scans SharePoint sites for files exceeding 500MB, compiles an HTML report with file details,
and sends the report via email using Microsoft Graph API. The report can be sent as an HTML email body or also as an HTML attachment.

.PARAMETER Configurable variables
- useAttachment   : 1 to send the report as both an HTML attachment and in the email body, 0 to send only in the email body.
- recipients      : Array of email addresses to receive the report.
- client1LogoUrl  : URL of the primary client logo displayed in the report footer.
- client2LogoUrl  : URL of the secondary client logo displayed in the report header.
...
[System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId((Get-Date), "Romance Standard Time") | ForEach-Object {
    $dateTime = $_.ToString("dd/MM/yyyy HH:mm")
}
