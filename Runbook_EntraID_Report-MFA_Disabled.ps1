<#
.SYNOPSIS
Report on the MFA (Multi-Factor Authentication) status of users in Azure AD, with colors based on the status.

.DESCRIPTION
This script retrieves the list of users from Microsoft Graph API,
checks the MFA authentication method status for each user,
and generates an HTML report highlighting whether MFA is enabled, not enabled, or if an error occurred.

.CONFIGURATION PARAMETERS
- GraphClientId: ID of the registered application in Azure AD.
- GraphTenantId: Tenant ID of Azure AD.
- GraphSecret: Application secret.
- No-Reply_Email: SMTP credentials to send the report.
- recipients: List of email recipients (one or more).
- useAttachment: 1 to send the HTML as an attachment and in the body, 0 for body only.
