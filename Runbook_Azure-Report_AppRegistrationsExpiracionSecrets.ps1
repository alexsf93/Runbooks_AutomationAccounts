<#
.SYNOPSIS
App Registrations report with secrets and their expiration dates, with row coloring based on proximity to expiration.

.DESCRIPTION
This script retrieves App Registrations and their secrets from the Microsoft Graph API,
and generates an HTML report highlighting secrets that are close to expiring, with row colors based on days remaining.

.PARAMETER Configurable variables
- GraphClientId: Registered App ID in Azure AD.
- GraphTenantId: Azure AD tenant ID.
- GraphSecret: App secret.
- Correo_No-Reply: SMTP credentials.
- recipient: Email recipients (can be one or multiple).
- usarAdjunto: 1 to send HTML as attachment and in body, 0 to send HTML only in the body.
...
$attachReport = 0  # 1 = send HTML attachment + body, 0 = only HTML in body
